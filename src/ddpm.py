...
class DDPM(torch.nn.Module):
    ...
    @torch.no_grad()
    def sample(self, model, n_sample, img_size, context_dim, timesteps, step_size, plot=False, sample_contexts=None):
        # sample initial noise
        samples = torch.randn(n_sample, 3, img_size, img_size).to(self.device)
        if sample_contexts is not None:
            sample_contexts = torch.tensor(sample_contexts).view(n_sample,1,context_dim).float().to(self.device)
        else:
